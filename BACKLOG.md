...
| Bug 19 | ✅ Fixed | Vocabulary icon in Learn path was `'◻'` (a near-invisible hollow white square against the light background). Changed to `'श'` (from शब्द, "word"), matching the existing pattern of a real Devanagari letter per topic. |
| Bug 20 | ✅ Fixed | PWA "Add to Home Screen" showed a blank/generic icon — `manifest.json` referenced `icons/icon-192.png` and `icons/icon-512.png` but neither file existed (404). Designed and added a new icon: सं (first syllable of संस्मरण) in Noto Serif Devanagari Bold on a maroon/gold radial-gradient circle. Added icon-192.png, icon-512.png, and a 180×180 apple-touch-icon.png, plus the missing `<link rel="apple-touch-icon">` tag in index.html's head. |
